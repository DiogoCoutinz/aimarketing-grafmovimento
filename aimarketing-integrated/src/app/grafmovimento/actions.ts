...
export async function analyzeImageA(projectId: string) {
  try {
    ...
    // ✅ USAR FUNCIONALIDADE JÁ EXISTENTE: Gerar sugestões AI
    const suggestions = await generateAISuggestions(projectId)
    
    // Criar análise estruturada compatível com o frontend
    const analysis = {
      productType: 'Produto Identificado',
      dominantColors: ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4'], // TODO: Implementar extração real de cores
      style: 'Análise baseada em OpenAI GPT-4o-mini',
      mood: 'Criativo e Profissional',
      suggestedScenarios: suggestions.map((suggestion, index) => ({
        id: `ai_suggestion_${index + 1}`,
        name: `Sugestão AI ${index + 1}`,
    ...
    // Atualizar status com análise real
    await updateGrafMovimentoProject(projectId, {
      status: 'image_a_analyzed',
      image_a_analysis: analysis
    })
    
    console.log('✅ Análise OpenAI concluída com', suggestions.length, 'sugestões')
...
- "Convert to a dreamy watercolor painting style with soft pastel tones and ethereal atmosphere"`
        },
        {
          ...
          content: [
            ...
            await (async () => {
              try {
                console.log('🔄 Baixando imagem para conversão base64:', project.image_a_url)
                const imageResponse = await fetch(project.image_a_url)
                ...
                console.error('❌ Erro ao buscar imagem, usando URL direta:', fetchError)
                return {
                  type: 'image_url',
                  image_url: {
  ...
  try {
    ...
    const { request_id } = await fal.queue.submit('fal-ai/bytedance/seedream/v4/edit', {
      ...
      webhookUrl: process.env.NODE_ENV === 'development' 
        ? undefined // Não usar webhook em desenvolvimento - fazer polling
        : `${process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'https://aimarketing-grafmovimento.vercel.app'}/api/grafmovimento/fal-webhook`
    })
